...
exports.STANDARD_MAJOR_SECTIONS = void 0;
exports.getStructuredMajorSections = getStructuredMajorSections;
// Standard JLPT Major Sections
exports.STANDARD_MAJOR_SECTIONS = [
    {
        id: "vocab",
 ...
 */
function getStructuredMajorSections(examData) {
    ...
    const extractMondaiInfo = (mondaiStr, fallbackMajorId = "vocab") => {
        if (!mondaiStr) {
            return {
                key: "mondai-general",
                num: "",
    ...
    const majorSections = [];
    exports.STANDARD_MAJOR_SECTIONS.forEach((std) => {
        const mondaisForSection = [];
        mondaiMap.forEach((m) => {
        ...
        if (mondaisForSection.length === 0) {
            sections.forEach((sec) => {
                let matchStd = false;
                if (std.id === "vocab" && sec.name.includes("語彙"))
                    matchStd = true;
                if (std.id === "grammar" && sec.name.includes("文法"))
                    matchStd = true;
                if (std.id === "reading" && sec.name.includes("読解"))
                    matchStd = true;
                if (std.id === "listening" && sec.name.includes("聴解"))
                    matchStd = true;
                if (matchStd) {
                    mondaisForSection.push({
                        id: `sec-${sec.name}`,
